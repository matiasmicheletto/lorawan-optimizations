...
OptimizationResults greedy2(Instance* l, Objective* o, bool verbose){
    ...
    for(uint s = 7; s <= 12; s++){ // For each spread factor        
        ...
        for(uint g = 0; g < gwCount; g++){ // Allocate end devices
            ...
            //std::cout << "SF = " << s << ", GW = " << g << ", eds = " << edList.size() << std::endl;
            for(uint e = 0; e < edList.size(); e++){
                uint selectedED = edList[e];
                //std::cout << "Selected = " << selectedED << std::endl;
                if(std::find(allocatedEDs.begin(), allocatedEDs.end(), selectedED) == allocatedEDs.end()){
                    allocatedEDs.push_back(selectedED);                    
                    gw[selectedED] = g;
                    sf[selectedED] = s;
                    //std::cout << " -- pushed: " << selectedED << std::endl;
                }
        ...
        if(allocatedEDs.size() < edCount){
            //std::cout << "SIZE = " << allocatedEDs.size() << std::endl;
...
OptimizationResults gre3(Instance* l, Objective* o, bool verbose) {
    auto start = std::chrono::high_resolution_clock::now();
